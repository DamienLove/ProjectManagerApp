...
class TestRemoteAgentAuth(unittest.TestCase):
    ...
    def test_auth_logic_invalid(self):
        ...
        # Resolve the actual exception class being used by the module (which might be clobbered by other tests)
        ActualHTTPException = sys.modules["fastapi"].HTTPException
        with self.assertRaises(ActualHTTPException):
            self.remote_agent.require_token_from_request(request)
