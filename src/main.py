...
class ProjectCard(ctk.CTkFrame):
    ...
    def _populate_controls(self):
        ...
        if self.status == "Local":
            ...
            self._btn("AntiG", lambda: self.app.open_antigravity(self.name), color="#9333ea", icon=self.app.icons.get("antigravity"), tooltip="Open Antigravity (Python Easter Egg)")
            self._btn("Config", lambda: ProjectConfigWindow(self.app, self.name, os.getenv("LOCAL_WORKSPACE_ROOT", DEFAULT_WORKSPACE)), color="#64748b", icon=self.app.icons.get("config_cog"), tooltip="Manage paths and software")
            self._btn("Deactivate", lambda: self.app.deactivate_project(self.name), color="#ef4444", icon=self.app.icons.get("cloud"), tooltip="Offload project to cloud & uninstall apps")
        else:
            self._btn("Activate", lambda: self.app.activate_project(self.name), color="#3b82f6", icon=self.app.icons.get("activate"), tooltip="Restore project from cloud & install apps")
            self._btn("Forget", lambda: self.app.forget_project(self.name), color="transparent", text_color="red", icon=self.app.icons.get("quit"), tooltip="Remove from list (files remain)")
